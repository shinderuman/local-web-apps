...
test('shiftSortOrders: 各アイテムのsortOrderを+1する', () => {
    const items = [{ id: 1, sortOrder: 0 }, { id: 2, sortOrder: 1 }, { id: 3, sortOrder: 2 }];
...
test('sortItems: 手動順（sortOrder昇順）', () => {
    ...
    assert.deepStrictEqual(result.map(i => i.sortOrder), [0, 1, 2]);
});

test('sortItems: タイトル順昇順', () => {
    const items = [{ title: 'ばなな' }, { title: 'あっぷる' }, { title: 'おれんじ' }];
    const result = sortItems(items, 'title', true);
    assert.deepStrictEqual(result.map(i => i.title), ['あっぷる', 'おれんじ', 'ばなな']);
});

test('sortItems: タイトル順降順', () => {
    const items = [{ title: 'ばなな' }, { title: 'あっぷる' }, { title: 'おれんじ' }];
    const result = sortItems(items, 'title', false);
    assert.deepStrictEqual(result.map(i => i.title), ['ばなな', 'おれんじ', 'あっぷる']);
});
...
test('sortItems: 登録順昇順（createdAt）', () => {
    ...
    assert.deepStrictEqual(result.map(i => i.createdAt), [100, 200, 300]);
});

test('sortItems: 登録順降順', () => {
    const items = [{ createdAt: 300 }, { createdAt: 100 }, { createdAt: 200 }];
    const result = sortItems(items, 'createdAt', false);
    assert.deepStrictEqual(result.map(i => i.createdAt), [300, 200, 100]);
});
...
test('isValidItemInput: 全て有効ならtrue', () => {
    assert.strictEqual(isValidItemInput('1', '2', 'タイトル', 'https://example.com'), true);
});

test('isValidItemInput: ウィンドウ値が空ならfalse', () => {
    assert.strictEqual(isValidItemInput('', '2', 'タイトル', 'https://example.com'), false);
});

test('isValidItemInput: グループ値が空ならfalse', () => {
    assert.strictEqual(isValidItemInput('1', '', 'タイトル', 'https://example.com'), false);
});

test('isValidItemInput: タイトルが空ならfalse', () => {
    assert.strictEqual(isValidItemInput('1', '2', '', 'https://example.com'), false);
});
...
test('stripSynopsisForExport: synopsisフィールドを除外', () => {
    const item = { id: 1, title: 't', url: 'u', synopsis: [{ volume: 1, caption: 'あらすじ' }] };
