...
test('実データ全件: parseVolume が期待値と一致', () => {
    titles.forEach(title => {
        ...
        assert.strictEqual(actual, exp, `巻数不一致: "${title.slice(0, 30)}..." expected=${exp} actual=${actual}`);
    });
});
...
test('実データ全件: parseBaseTitle がメインタイトル先頭を保持', () => {
    titles.forEach(title => {
        ...
        assert.ok(head === '' || base.startsWith(head), `先頭が保持されていない: "${title.slice(0, 30)}..." base="${base}" head="${head}"`);
    });
});
