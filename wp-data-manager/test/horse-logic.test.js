...
test('createHorse: 既存の最大orderの次に必要な初期値を持つレコードを作る', () => {
    const existing = [{ id: 1, order: 2 }, { id: 2, order: 5 }];
    const result = createHorse({ name: '新系統', birthYear: '1990', horseName: '' }, existing, 99);
...
test('createHorse: 生年未入力は空文字のまま保持する', () => {
    const result = createHorse({ name: '新系統', birthYear: '', horseName: '所有馬' }, [], 99);
...
test('updateHorseValue: 指定レコードだけを更新し、元レコードを変更しない', () => {
    const horses = [{ id: 1, horseName: '旧名' }, { id: 2, horseName: '維持' }];
    const result = updateHorseValue(horses, 1, 'horseName', '新名');

    assert.deepStrictEqual(result, [{ id: 1, horseName: '新名' }, { id: 2, horseName: '維持' }]);
    assert.deepStrictEqual(horses, [{ id: 1, horseName: '旧名' }, { id: 2, horseName: '維持' }]);
});

test('toggleHorseRunner: 閾値未満なら指定レコードの現役状態だけを反転する', () => {
    const horses = [{ id: 1, birthYear: 1995, isRunner: true }, { id: 2, birthYear: 1995, isRunner: true }];
    const result = toggleHorseRunner(horses, 1, 2000);

    assert.deepStrictEqual(result, [{ id: 1, birthYear: 1995, isRunner: false }, { id: 2, birthYear: 1995, isRunner: true }]);
...
test('getNextSortState: 同じ列は昇降順を反転し、別列は昇順で開始する', () => {
    assert.deepStrictEqual(getNextSortState({ key: 'name', asc: true }, 'name'), { key: 'name', asc: false });
    assert.deepStrictEqual(getNextSortState({ key: 'name', asc: false }, 'birthYear'), { key: 'birthYear', asc: true });
});
...
test('sortHorses: 未入力値を末尾にして日本語名を昇順に並べ、元配列を変更しない', () => {
    ...
    assert.deepStrictEqual(result.map(horse => horse.name), ['馬A', '馬B', '']);
    assert.deepStrictEqual(horses.map(horse => horse.name), ['馬B', '', '馬A']);
});

test('reorderHorses: 移動後の表示順に連番orderを振り直し、元配列を変更しない', () => {
    const horses = [{ id: 1, order: 1 }, { id: 2, order: 2 }, { id: 3, order: 3 }];
    const result = reorderHorses(horses, 0, 2);

    assert.deepStrictEqual(result, [{ id: 2, order: 1 }, { id: 3, order: 2 }, { id: 1, order: 3 }]);
    assert.deepStrictEqual(horses, [{ id: 1, order: 1 }, { id: 2, order: 2 }, { id: 3, order: 3 }]);
});
...
test('listHistoricalHorseGroups: ゲーム内年の前年以降を年順で返し、元データを変更しない', () => {
    ...
    assert.deepStrictEqual(result, [{ year: 1968, horses: ['馬A'] }, { year: 1970, horses: ['馬B'] }]);
    assert.deepStrictEqual(master, { 1968: ['馬A'], 1970: ['馬B'], 1967: ['馬C'] });
});
