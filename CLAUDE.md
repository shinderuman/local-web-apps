...
- 純粋関数がアプリ固有の定数（ドメイン知識。例: 用語解説のグロッサリー）を必要とする場合、**定数は dom-helpers 側に持ち込まず app.js に置き、引数で注入する**。dom-helpers は汎用DOM部品としてドメイン知識に依存しない状態を保つ。状態参照・イベント結びつけと同様に、定数依存も引数注入で切れるため切り出し対象から外す理由にはならない
