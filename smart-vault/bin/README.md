...
Time Machine 用ストレージの APFS 構造（履歴・スナップショット・Backupロール）ごと別ディスクへクローンする。ddrescue でブロックコピーし、UUID 重複を避けるためコピー後に一度両方取り外してコピー先だけ再接続する。リサイズ失敗時は履歴を段階的に間引き（24時間/週次/月次/1件ずつ）、最終的に APFS 新規作成へフォールバック。コピー先がコピー元より小さい場合は、APFS が提示する最小縮小サイズ＋5GB までコピー元を縮小してコピーし、完了後に元の最大容量へ戻す。
...
- コピー先が小さい場合、`resizeContainer limits` で得た APFS の最小縮小サイズに 5GB の安全余白を足したサイズへコピー元を縮小してコピーする（実使用量が少なければ小さく済む）。完了後はコピー先・コピー元の順に取り外す間にコピー元を最大容量へ再拡張し、再接続後にコピー先を全体へ拡張してから初回バックアップを取る
