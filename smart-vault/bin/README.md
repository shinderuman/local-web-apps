...
Time Machine 用ストレージの APFS 構造（履歴・スナップショット・Backupロール）ごと別ディスクへクローンする。ddrescue でブロックコピーし、UUID 重複を避けるためコピー後に一度両方取り外してコピー先だけ再接続する。リサイズ失敗時は履歴を段階的に間引き（24時間/週次/月次/1件ずつ）、最終的に APFS 新規作成へフォールバック。
