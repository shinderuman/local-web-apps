...
((root, factory) => {
    ...
    const LEVEL_SCORE_MAX = [0, 9, 99, 999, 9999, 99999, Number.MAX_SAFE_INTEGER];
    ...
    const makeScorer = () => {
        ...
        return {
            add,
            getScore: () => score,
            getReasons: () => (reasons.length === 0 ? ['判定基準に該当なし'] : reasons)
        };
    };
    ...
    const judgeHdd = (ctx, add) => {
        ...
        if (pendingSectors > 0) add(pendingSectors * SECTOR_WEIGHT.pending, `保留中セクタ(197)=${pendingSectors}`);
        if (offlineUncorrectable > 0) add(offlineUncorrectable * SECTOR_WEIGHT.uncorrectable, `回復不能セクタ(198)=${offlineUncorrectable}`);
        if (reallocSectors > 0) add(reallocSectors * SECTOR_WEIGHT.realloc, `代替処理済セクタ(5)=${reallocSectors}`);
        if (errs.hasUNC) add(errs.errCount * HDD_PENALTY.uncPerError, `エラーログにUNC検出 (${errs.errCount}件)`);
        if (errs.hasIDNF) add(HDD_PENALTY.idnf, 'エラーログにIDNF検出');

        if (hours_val >= HDD_PENALTY.hourBenchmark) {
            add(hours_val - HDD_PENALTY.hourBenchmark, `通電時間=${hours_val}H (公称寿命${HDD_PENALTY.hourBenchmark}H超)`);
        }

        if (crcErrors > 0) add(crcErrors, `UDMA_CRC(199)=${crcErrors}`);
        if (commandTimeout > 0) add(HDD_PENALTY.commandTimeout, `Command_Timeout(188)=${commandTimeout}`);
    ...
    const judgeSataSsd = (ctx, add) => {
        const { table, hours_val, lifePercent, reallocSectors, crcErrors, tbw_val } = ctx;

        const lifeKnown = (lifePercent >= 0 && lifePercent <= 100);
        if (lifeKnown) {
            add((100 - lifePercent) * SSD_PENALTY.lifePerPoint, `残り寿命=${lifePercent}%`);
            if (lifePercent <= 10) add(SSD_PENALTY.lifeCritical, `残り寿命=${lifePercent}% (<=10)`);
        } else if (!tbw_val) {
            // 残り寿命も書込量も取得不能は評価不能＝異常扱い
            add(SSD_PENALTY.unknownLife, '残り寿命・書込容量ともに取得不能');
        }

        const reserved = (table || []).find(a => a.id === 232 || a.id === 233);
        if (reserved && reserved.thresh !== undefined && Number(reserved.value) < Number(reserved.thresh)) {
            add(SSD_PENALTY.reservedLow, `予備ブロック(ID${reserved.id})がしきい値下回り`);
        }

        if (reallocSectors > 0) add(reallocSectors * SECTOR_WEIGHT.realloc, `代替処理済セクタ(5)=${reallocSectors}`);
        if (crcErrors > 0) add(crcErrors, `UDMA_CRC(199)=${crcErrors}`);
        if (hours_val >= SSD_PENALTY.hourBenchmark) {
            add(hours_val - SSD_PENALTY.hourBenchmark, `通電時間=${hours_val}H (公称寿命${SSD_PENALTY.hourBenchmark}H超)`);
        }
    };
    ...
    const judgeNvme = (nvmeLog, add) => {
        ...
        if (criticalWarning !== 0) add(NVME_PENALTY.criticalWarning, `critical_warning=0x${criticalWarning.toString(16)}`);
        if (mediaErrors > 0) add(mediaErrors * NVME_PENALTY.mediaErrorPerCount, `media_errors=${mediaErrors}`);
        if (!Number.isNaN(pctUsed) && pctUsed >= NVME_PENALTY.pctUsedCriticalThreshold) {
            add((pctUsed - NVME_PENALTY.pctUsedCriticalThreshold) * NVME_PENALTY.pctUsedPerPoint + NVME_PENALTY.pctUsedCritical, `percentage_used=${pctUsed}% (>=${NVME_PENALTY.pctUsedCriticalThreshold})`);
        }
        if (!Number.isNaN(availSpare)) {
            if (availSpare <= NVME_PENALTY.availSpareCritical) add(NVME_PENALTY.availSpareLow, `available_spare=${availSpare}% (<=${NVME_PENALTY.availSpareCritical})`);
            if (availSpare > NVME_PENALTY.availSpareCritical && availSpare <= NVME_PENALTY.availSpareWarnMax) {
                add((NVME_PENALTY.availSpareWarnMax - availSpare) * NVME_PENALTY.availSpareWarnPerPoint, `available_spare=${availSpare}% (${NVME_PENALTY.availSpareCritical + 1}-${NVME_PENALTY.availSpareWarnMax})`);
            }
        }
    };
    ...
    const dispatchByType = (data, record, add) => {
        const { customType, hours_val, lifePercent, reallocSectors, pendingSectors, crcErrors, tbw_val } = record;
        const table = data.ata_smart_attributes?.table || [];
        const errs = parseErrorDescriptions(data.ata_smart_error_log?.summary);
        const ctx = { table, hours_val, lifePercent, reallocSectors, pendingSectors, crcErrors, tbw_val, errs };
