...
test('HDD: エラーログ UNC は件数×5、IDNF は+3000', () => {
    const data = {
        ...
        ata_smart_error_log: {
            summary: {
                count: 13,
                table: [{ error_description: 'Error: UNC at LBA' }]
            }
        }
    };
    const r = computeSeverityScore(data, {
...
test('HDD: Command_Timeout>0 は+500固定（生値不使用）', () => {
    const data = {
        ata_smart_attributes: {
            table: [{ id: 188, raw: { value: 566944400149 } }]
        },
        ata_smart_error_log: { summary: { count: 0 } }
    };
    const r = computeSeverityScore(data, {
...
test('parseErrorDescriptions: UNCを検出', () => {
    const summary = {
        count: 1,
        table: [{ error_description: 'Error: UNC at LBA = 0x008b19c6' }]
    };
...
test('parseErrorDescriptions: ICRC/ABRTを検出', () => {
    const summary = {
        count: 6,
        table: [{ error_description: 'Error: ICRC, ABRT at LBA = 511' }]
    };
    const r = parseErrorDescriptions(summary);
