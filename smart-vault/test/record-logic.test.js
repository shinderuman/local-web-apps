...
test('countRecordsByType: フィルタ種別ごとの件数と全件数を返す', () => {
    ...
    const result = countRecordsByType(records, ['all', 'nvme', 'hdd-35', 'unknown']);

    assert.deepStrictEqual(result, { all: 3, nvme: 1, 'hdd-35': 1, unknown: 1 });
});
...
test('sortRecords: 数値列を昇順に並べ、元配列を変更しない', () => {
    const records = [{ id: 1, severityScore: 5 }, { id: 2, severityScore: null }, { id: 3, severityScore: 2 }];
    const result = sortRecords(records, 'severityScore', 'asc');

    assert.deepStrictEqual(result.map(record => record.id), [2, 3, 1]);
    assert.deepStrictEqual(records.map(record => record.id), [1, 2, 3]);
});
...
test('reorderRecordsByVisiblePosition: 表示中レコードの移動を全体配列へ反映する', () => {
    ...
    const result = reorderRecordsByVisiblePosition(records, visibleRecords, 3, 1, 0);

    assert.deepStrictEqual(result.map(record => record.id), [3, 1, 2, 4]);
    assert.deepStrictEqual(records.map(record => record.id), [1, 2, 3, 4]);
});
